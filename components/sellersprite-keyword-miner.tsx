...
export function SellerSpriteKeywordMiner({ onImportKeywords }: SellerSpriteKeywordMinerProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const isAsin = searchType === "asin" || /^B[A-Z0-9]{9}$/i.test(queryInput.trim());

      const res = await fetch("/api/keywords/sellersprite", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          asin: isAsin ? queryInput.trim() : undefined,
          keyword: !isAsin ? queryInput.trim() : undefined,
        ...
        <div className="pt-4 grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
          ...
          <div className="sm:col-span-6 space-y-1">
            <label className="block text-xs font-bold text-slate-600">
              {searchType === "asin" ? "Nhập ASIN đối thủ (ví dụ: B0GTQQMFVM):" : "Nhập Seed Keyword (ví dụ: acrylic ornament):"}
            </label>
            <div className="relative">
              <input
                ...
                placeholder={searchType === "asin" ? "Ví dụ: B0GTQQMFVM, B08CHL89VP..." : "Ví dụ: acrylic ornament, christmas gift..."}
