...
log_file = f"E:\\PythonProject1\\Amzdata-05-campaign-{current_date}.txt"
...
def create_output_directory(output_base_directory, store_name, range_day_report, campaign_type):
    folder_name = f"{current_date}-{SIGNATURE}" if USE_SIGNATURE else current_date
    date_directory = os.path.join(output_base_directory, folder_name)
    os.makedirs(date_directory, exist_ok=True)
    store_directory = os.path.join(date_directory, store_name)
    os.makedirs(store_directory, exist_ok=True)
    range_directory = os.path.join(store_directory, range_day_report)
    os.makedirs(range_directory, exist_ok=True)
    current_time = datetime.now().strftime("%Y-%m-%d")
    final_directory = os.path.join(range_directory, f"Campaign File {campaign_type} {current_time}")
    os.makedirs(final_directory, exist_ok=True)
    return final_directory


def filter_and_save_excel_sb(input_file, output_directory, range_day_report, store_name):
    try:
        logging.info(f"[{store_name}] Đang xử lý SB cho file: {os.path.basename(input_file)}")
        df_combined = pd.DataFrame()
        xls = pd.ExcelFile(input_file, engine='openpyxl')

        # --- START: SỬA LỖI HOA/THƯỜNG ---
        sheet_name_keyword = "sponsored brands campaigns"
        sheet_names = [sheet_name for sheet_name in xls.sheet_names if sheet_name_keyword in sheet_name.lower()]
        ...
        if df_combined.empty or 'portfolio name (informational only)' not in df_combined.columns:
            logging.info(
                f"[{store_name}] Không có dữ liệu SB hợp lệ hoặc thiếu cột Portfolio Name trong file: {os.path.basename(input_file)}")
            return
        ...
        for sku in skus:
            ...
            # --- START: SỬA LỖI HOA/THƯỜNG (cần kiểm tra cột 'match type' tồn tại) ---
            if 'match type' not in sku_data.columns:
                continue
            ...
            if not collection_data.empty:
                output_file = os.path.join(output_directory, f"{safe_sku} Bulk Collection {range_day_report}.xlsx")
                collection_data_sorted = collection_data.sort_values(by='spend', ascending=False)
                write_excel_with_retry(collection_data_sorted, output_file, 'Collection')

            video_condition = sku_data['campaign name (informational only)'].str.contains(r'\b(?:sb03|video)\b',
                                                                                          case=False, regex=True,
                                                                                          na=False)
            video_data = sku_data[video_condition & filter_condition]
            if not video_data.empty:
                output_file = os.path.join(output_directory, f"{safe_sku} Bulk Video {range_day_report}.xlsx")
                video_data_sorted = video_data.sort_values(by='spend', ascending=False)
                write_excel_with_retry(video_data_sorted, output_file, 'Video')

    except Exception as e:
        logging.error(f"[{store_name}] Lỗi khi xử lý SB cho file {os.path.basename(input_file)}: {e}")
        msg = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_name)}*\n" \
              f"❌ _Lỗi xử lý Campaign SB_\n" \
              f"Lỗi: `{escape_markdown(e)}`"
        send_telegram_message(msg)


def filter_and_save_excel_sp(input_file, output_directory, range_day_report, store_name):
    try:
        logging.info(f"[{store_name}] Đang xử lý SP cho file: {os.path.basename(input_file)}")
        df_combined = pd.DataFrame()
        xls = pd.ExcelFile(input_file, engine='openpyxl')

        # --- START: SỬA LỖI HOA/THƯỜNG ---
        sheet_name_keyword = "sponsored products campaigns"
        sheet_names = [sheet_name for sheet_name in xls.sheet_names if sheet_name_keyword in sheet_name.lower()]
        # --- END: SỬA LỖI HOA/THƯỜNG ---
        ...
        if df_combined.empty or 'portfolio name (informational only)' not in df_combined.columns:
            logging.info(
                f"[{store_name}] Không có dữ liệu SP hợp lệ hoặc thiếu cột Portfolio Name trong file: {os.path.basename(input_file)}")
            return
        ...
        for sku in skus:
            ...
            if 'match type' not in sku_data.columns:
                continue
            filter_condition = sku_data['campaign name (informational only)'].str.contains('Exact|Phrase|Broad',
                                                                                           case=False, na=False)
            filter_condition &= sku_data['match type'].isin(['Exact', 'Phrase', 'Broad'])
            # --- END: SỬA LỖI HOA/THƯỜNG ---

            filtered_data = sku_data[filter_condition]
            if not filtered_data.empty:
                output_file = os.path.join(output_directory, f"{safe_sku} Campaign File SP {range_day_report}.xlsx")
                filtered_data_sorted = filtered_data.sort_values(by='spend', ascending=False)
                write_excel_with_retry(filtered_data_sorted, output_file, 'Keyword')
            ...
            if 'product targeting expression' in sku_data.columns:
                ...
                if not filtered_data_auto.empty:
                    output_file = os.path.join(output_directory,
                                               f"{safe_sku} Campaign File SP {range_day_report} Auto.xlsx")
                    filtered_data_auto_sorted = filtered_data_auto.sort_values(by='spend', ascending=False)
                    write_excel_with_retry(filtered_data_auto_sorted, output_file, 'Auto')

    except Exception as e:
        logging.error(f"[{store_name}] Lỗi khi xử lý SP cho file {os.path.basename(input_file)}: {e}")
        msg = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_name)}*\n" \
              f"❌ _Lỗi xử lý Campaign SP_\n" \
              f"Lỗi: `{escape_markdown(e)}`"
        send_telegram_message(msg)
...
def process_store(args):
    ...
    logging.info(f"--- Bắt đầu xử lý Campaign cho store: {store_folder} ---")
    msg = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
          f"🚀 _Bắt đầu xử lý Campaign_"
    send_telegram_message(msg)
    store_start_time = time.time()

    try:
        has_sb = any("Bulk File SB" in f for f in os.listdir(store_path))
        has_sp = any("Bulk File SP" in f for f in os.listdir(store_path))

        if has_sp:
            sp_start_time = time.time()
            msg_sp_start = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
                           f"⏳ _Bắt đầu xử lý Campaign SP\\.\\.\\._"
            ...
            for file in [f for f in os.listdir(store_path) if "Bulk File SP" in f]:
                ...
                output_directory_sp = create_output_directory(output_base, store_folder, range_day_report, "SP")
                filter_and_save_excel_sp(input_file_path, output_directory_sp, range_day_report, store_folder)
            ...
            msg_sp_end = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
                         f"✅ _Hoàn thành xử lý Campaign SP_\n" \
        ...
        if has_sb:
            ...
            msg_sb_start = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
                           f"⏳ _Bắt đầu xử lý Campaign SB\\.\\.\\._"
            send_telegram_message(msg_sb_start)
            for file in [f for f in os.listdir(store_path) if "Bulk File SB" in f]:
                input_file_path = os.path.join(store_path, file)
                parts = file.split(" ")
                range_day_report = parts[-2][1:] + " " + parts[-1][:-6]
                output_directory_sb = create_output_directory(output_base, store_folder, range_day_report, "SB")
                filter_and_save_excel_sb(input_file_path, output_directory_sb, range_day_report, store_folder)
            ...
            msg_sb_end = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
                         f"✅ _Hoàn thành xử lý Campaign SB_\n" \
        ...
        logging.info(
            f"--- Hoàn thành xử lý Campaign cho store {store_folder} trong {format_duration(store_duration)} ---")
        msg = f"*{escape_markdown(SCRIPT_NAME)}* \\- *{escape_markdown(store_folder)}*\n" \
              f"🎉 _Hoàn thành xử lý Campaign_\n" \
...
if __name__ == '__main__':
    ...
    end_msg = f"✅ *{escape_markdown(SCRIPT_NAME)}* \\- _Hoàn thành xử lý Campaign cho tất cả stores_\n" \
